...
void DFPD_g_estimate(vector < vector<int> >&estimate_v, vector < vector<int> >&estimate_h, Mat &test)
{
	...
	for (int i = 2; i < row - 2; i++)
		for (int j = 2; j < col - 2; j++)
		{
			if ((i + j) % 2 == 0)
			{
				estimate_h.at(i).at(j) = (((int)test.at<uchar>(i, j + 1) + (int)test.at<uchar>(i, j - 1)) / 2) + ((2 * (int)test.at<uchar>(i, j) - (int)test.at<uchar>(i, j + 2) - (int)test.at<uchar>(i, j - 2)) / 4);//灰階用uchar
				estimate_v.at(i).at(j) = (((int)test.at<uchar>(i + 1, j) + (int)test.at<uchar>(i - 1, j)) / 2) + ((2 * (int)test.at<uchar>(i, j) - (int)test.at<uchar>(i + 2, j) - (int)test.at<uchar>(i - 2, j)) / 4);
...
void DFPD_color_diffrence(vector < vector<int> >  &estimate_v, vector < vector<int> >  &estimate_h, vector < vector<int> >  &estimate_v_c, vector < vector<int> >  &estimate_h_c, Mat &test)
{
	...
	for (int i = 2; i < row - 2; i++)
		for (int j = 2; j < col - 2; j++)
		{
			if ((i + j) % 2 == 0)
			{
				estimate_h_c.at(i).at(j) = (int)test.at<uchar>(i, j) - estimate_h.at(i).at(j);
				estimate_v_c.at(i).at(j) = (int)test.at<uchar>(i, j) - estimate_v.at(i).at(j);
			}
		}
}
...
void fuzzy_smooth_method(vector < vector<int> >  &estimate_green, vector < vector<double> >  &estimate_s, Mat &test, int i, int j)
{
	...
	estimate_green.at(i).at(j) = (int)test.at<uchar>(i, j) + (fuzzy_g - fuzzy_b_r);
